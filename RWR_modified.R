...
library(Rgraphviz)
set.seed(8)
disease_name_list = c('total_IIMs','PM','DM','JDM','anti_Jo1')
for (disease_name in disease_name_list){
  #Read disease input file
  disease_gene_data_frame_z_score = read.table(paste0(disease_name,'_z_score.txt'),head=T)
  disease_gene_data_frame_z_score$abs_Z_score = abs(disease_gene_data_frame_z_score$Z_score)
  disease_gene_data_frame_z_score$Lead_SNP = gsub('-','.',disease_gene_data_frame_z_score$Lead_SNP )
  ...
  t3 = read.csv(paste0(disease_name,'_Coexpression_immune_tissue_cell_line_network_edgelist_corr0.75_adjacency_matrix_undirected.csv'),head=T)
  rownames(t3)=t3[,1]
  t3 = t3[c(2:(dim(t3)[2]))]
  t3 = as.matrix(t3)
  ...
  Total_Strengh_of_Nodes =colSums(SupraAdjacencyMatrix)
  Supra_Adj_Matrix_Normalized =  t(t(SupraAdjacencyMatrix)/(Total_Strengh_of_Nodes))
  
  colnames(Supra_Adj_Matrix_Normalized) = colnames(SupraAdjacencyMatrix)
  rownames(Supra_Adj_Matrix_Normalized) = rownames(SupraAdjacencyMatrix)
  
  tau = rep(1,L)/L
  Seeds_Genes_Scores = numeric(length = length(disease_gene_data_frame_z_score$Lead_SNP)*L)
  Seed_Genes_Layer_Labeled = character(length = length(disease_gene_data_frame_z_score$Lead_SNP)*L)
  
  Current_Gene_Labeled = character()
  
  for (k in 1:L){
    Current_Gene_Labeled = c(Current_Gene_Labeled,paste(disease_gene_data_frame_z_score$Lead_SNP[k],k,sep="_",collapse = "") )
    for (j in 1:length(disease_gene_data_frame_z_score$Lead_SNP)){
      Seed_Genes_Layer_Labeled[((k-1)*length(disease_gene_data_frame_z_score$Lead_SNP))+ j] = paste(disease_gene_data_frame_z_score$Lead_SNP[j],k,sep="_",collapse = "") 
      Seeds_Genes_Scores[((k-1)*length(disease_gene_data_frame_z_score$Lead_SNP))+ j] = tau[k]*disease_gene_data_frame_z_score$abs_Z_score[j]/sum(disease_gene_data_frame_z_score$abs_Z_score)
    }
  }  
  Seeds_Score = data.frame(Seeds_ID = Seed_Genes_Layer_Labeled,
                            Score = Seeds_Genes_Scores, stringsAsFactors = FALSE)
  
  NetworkSize = ncol(Supra_Adj_Matrix_Normalized)
  
  prox_vector = matrix(0,nrow = NetworkSize,ncol=1)
  
  seed_pos = which(colnames(Supra_Adj_Matrix_Normalized) %in% Seeds_Score[,1])
  for (i in seed_pos){
    prox_vector[i] = Seeds_Score$Score[which(Seeds_Score$Seeds_ID == colnames(Supra_Adj_Matrix_Normalized)[i])]
  }
  ...
  rwr <- function(Adjacency_matrix, prox_vector, alpha, Threeshold, err,iter){
    ...
    while(err >= Threeshold){
      ...
      prox_vector = (1-alpha)*(Supra_Adj_Matrix_Normalized %*% prox_vector) + alpha*restart_vector
  ...
  prox_vector = as.matrix(rwr(Supra_Adj_Matrix_Normalized, prox_vector, alpha, Threeshold, err,iter))
  ...
  Random_Walk_Results$log_value = log(as.numeric(Random_Walk_Results$V2))
  ...
  write.table(rank_global, paste0('RWR_Zscore_norm_modification/',disease_name,'_RWR_M_method_4_layer_PPI_KEGG_PharmGKB_Coexpression_gene_1e-15.txt'),quote = F,row.names=F,col.names=T)
  
  ## We sort the genes according to their score. 
  
  rank_global_NoSeeds = rank_global[which(!rank_global$rank_global_names %in% disease_gene_data_frame_z_score$Lead_SNP),]
  write.table(rank_global_NoSeeds,paste0('RWR_Zscore_norm_modification/',disease_name,'_No_Seeds_RWR_M_method_4_layer_PPI_KEGG_PharmGKB_Coexpression_gene_1e-15.txt'),quote = F,row.names=F,col.names=T)
  ...
  for (j in 1:nperms) {
    ...
    perm_rwr_ind = as.matrix(rwr(Supra_Adj_Matrix_Normalized, s_perm, alpha, Threeshold, err,iter))
    perm_rwr_ind = as.data.frame(cbind(rownames(perm_rwr_ind),perm_rwr_ind))
    perm_rwr_ind $V1 = gsub("_[^_]*$","",Random_Walk_Results$V1)
    perm_rwr_ind $V2 = log(as.numeric(perm_rwr_ind$V2))
  ...
  rank_global_result_sort_no_seed = rank_global_result_sort[which(!rank_global_result_sort$rank_global_names %in% disease_gene_data_frame_z_score$Lead_SNP),]
  
  write.table(rank_global_result_sort,paste0('RWR_Zscore_norm_modification/',disease_name,'_permutation_result.txt'),quote = F,row.names=F,col.names=F)
  
  write.table(rank_global_result_sort_no_seed,paste0('RWR_Zscore_norm_modification/',disease_name,'_permutation_no_seed_result.txt'),quote = F,row.names=F,col.names=F)
  ...
  write.table(rank_global_result_sort_no_seed_sig,paste0('RWR_Zscore_norm_modification/',disease_name,'_permutation_result_significant.txt'),quote = F,row.names=F,col.names=F)
  c = rank_global_result_sort_no_seed_sig$rank_global_names[1:30]
  c = append(c,disease_gene_data_frame_z_score$Lead_SNP)
  write.table(c, paste0('RWR_Zscore_norm_modification/Top30_genes_and_disease_genes_',disease_name,'_3_Network_PPI_KEGG_Coexpression_immune_gene_1e-15.txt'),quote = F,row.names=F,col.names=F)
}
